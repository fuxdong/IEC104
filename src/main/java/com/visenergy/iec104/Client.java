...
import com.visenergy.iec104.rabbitTask.ConsumerZzTask;
...
public class Client {
    ...
    public static void main(String args[]) {
        try {
            ...
            Init.start();
            // 向本机的2405端口发出客户请求
            Socketer socketer = Socketer.init("192.168.100.110", 2405);
//            Socketer socketer = Socketer.init("192.168.100.193", 2405);
            RabbitMq mq = RabbitMq.init();
            ConsumerZzTask task = new ConsumerZzTask(socketer, mq);
            Thread thread = new Thread(task);
            thread.start();
            socketer.sendZzCommand();

            while (true) {
                try {
                    if (socketer.getSocketInputStream() != null) {
                        Apdu apdu = new Apdu(new DataInputStream(socketer.getSocketInputStream()));
                        if (apdu.getApciType() == Apdu.ApciType.I_FORMAT) {
                            ...
                            socketer.sendConfirmCommand("68040100" + recStr);
                        } else if (apdu.getApciType() == Apdu.ApciType.STARTDT_ACT) {
                            socketer.sendConfirmCommand("68040B000000");
                        } else if (apdu.getApciType() == Apdu.ApciType.STOPDT_ACT) {
                            socketer.sendConfirmCommand("680423000000");
                        } else if (apdu.getApciType() == Apdu.ApciType.TESTFR_ACT) {
                            socketer.sendConfirmCommand("680483000000");
                        } else {
    ...
    public static void setByReflect(Object obj, String methodName, Object methodValue, String mt) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        ...
        if (!"set".equals(methodName)) {
            if ("DECIMAL".equals(mt)) {
                ...
                log.debug("不存在的方法类型，无法生成反射方法。");
            }
        } else {
            log.debug("YcObject对象中不存在该set方法");
        }
